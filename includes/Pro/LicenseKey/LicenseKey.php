...
final class LicenseKey {
	...
	/**
	 * Withdraws an already-activated license — Revoked, leaving
	 ...
	 * `snw_license_revoke_on_refund` setting decides whether a caller
	 * ever calls this at all — see Pro\LicenseKey\CancellationHandler.
	 */
	public static function revoke_serial( int $serial_id ): bool {
		...
		/**
		 * Fires after a license is revoked (order cancelled/refunded).
		 *
		 * @param int $serial_id
		 */
		do_action( 'snw_license_revoked', $serial_id );
